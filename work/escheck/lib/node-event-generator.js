 ...
 */
 ...
 // ESQuery 是一个库，允许像css那样查找节点通过节点名称或者节点属性查找AST节点
 // https://estools.github.io/esquery/ demo
 // 给一个节点配置多个校验规则，用于以css选择器的方式选择多种规则，制造发布订阅方式
 // Esprima是一个高性能、符合标准的 ECMAScript解析器
  ...
  * @property {boolean} isExit `true` if this should be emitted when exiting the node rather than when entering  是否应该在退出节点而不是进入节点时发出
 ...
 function countClassAttributes (parsedSelector) {
   switch (parsedSelector.type) {
     case 'child':
     case 'descendant': // 后代选择器
     case 'sibling':
     case 'adjacent': // 相邻兄弟选择器
...
// 2048
 ...
 class NodeEventGenerator {
    ...
    * @returns {void}
    */
   applySelectors (node, isExit, path) {
