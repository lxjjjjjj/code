...
// plugin 是 接入插件
 ...
 const runRules = require('./lib/check')
 ...
 class EsCheckPlugin {
   ...
   apply (compiler) {
     // thisCompilation 的说明 Executed while initializing the compilation, 
     // right before emitting the compilation event. 
     // This hook is not copied to child compilers.
     compiler.hooks.thisCompilation.tap('EsCheckPlugin', (compilation) => {
       // assetPath 的说明 确定文件路径
     ...
     compiler.hooks.afterEmit.tapPromise({
       ...
       if (allProblems.length) {
         ...
         await writeFilePromise(esCheckFilePath, '')
       }
     })
   }
 }
