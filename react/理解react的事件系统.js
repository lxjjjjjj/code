...
// codepen运行地址 https://codepen.io/pen?editors=0011
...
// "window"
// 解释上述执行事件最开始从原生 DOM 按钮一路冒泡到 body，body 的事件处理器执行，输出 body。注意此时流程还没进入 React。为什么？因为 React 监听的是 document 上的事件。
// 继续往上事件冒泡到 document。
...
// React17版本之后输出为 "react:button"

// 文档记载React17对事件委派的更改https://reactjs.org/blog/2020/08/10/react-v17-rc.html#changes-to-event-delegation
// React 17 之后将原有的document处理事件移到了rootNode上监听执行，
// 这样对于一个项目有多个版本的React有好处并且也不会影响jquery等库的监听
// 由于这一变化，现在将由一个版本管理的 React 树嵌入到由不同 React 版本管理的树中会更加安全。请注意，要使其正常工作，两个版本都需要为 17 或更高版本，这就是升级到 React 17 很重要的原因。
// 这一变化还使得将 React 嵌入到使用其他技术构建的应用程序中变得更加容易。例如，如果您的应用程序的外部“外壳”是用 jQuery 编写的，但其中的较新代码是用 React 编写的，e.stopPropagation()那么 React 代码内部现在会阻止它访问 jQuery 代码——正如您所期望的那样。这也适用于另一个方向。如果你不再喜欢 React 并且想要重写你的应用程序——例如，在 jQuery 中——你可以开始将外壳从 React 转换为 jQuery，而不会中断事件传播。
// 原文：React 将不再在document级别附加事件处理程序。相反，它会将它们附加到渲染 React 树的根 DOM 容器中：
// 实现合成事件的目的如下：
// 合成事件首先抹平了浏览器之间的兼容问题，另外这是一个跨浏览器原生事件包装器，赋予了跨浏览器开发的能力；
// 对于原生浏览器事件来说，浏览器会给监听器创建一个事件对象。如果你有很多的事件监听，那么就需要分配很多的事件对象，造成高额的内存分配问题。
// 如果在 ReactDOM.render() 之前的的 document 事件处理器上将冒泡结束掉，同样会影响 React 的执行。只不过这里需要调用的不是 event.stopPropagation()，而是 event.stopImmediatePropagation()。
// document.addEventListener("click", event => {
...
// 如何解决 因为 button 事件处理器的执行前提是事件达到 document 被 React 接收到，然后 React 将事件派发到 button 组件。既然在按钮的事件处理器执行之前，事件已经达到 document 了，那当然就无法在按钮的事件处理器进行阻止了。
...
// react17之前 即使组织了冒泡依然会执行 console.log('propagation')
// React（当前）在document为（几乎）所有事件使用侦听器，因此当您的组件接收到事件时，
// 它已经一直冒泡到document并停止它只会阻止它通过 React 综合冒泡等级制度。
