# 总结
首先通过 processAttrs 解析标签的属性值 如果是 事件指令 再通过 parseModifiers 解析出事件修饰符。然后通过 addHandler 函数首先根据 modifier 修饰符对事件名 name 做处理，接着根据 modifier.native 判断是一个纯原生事件还是普通事件，分别对应 el.nativeEvents 和 el.events，最后按照 name 对事件做归类，并把回调函数的字符串保留到对应的事件中。addHandler 中接收 important 参数，可以按照优先级将hander排在handler数组的前面。然后在 codegen 的阶段，会在 genData 函数中根据 AST 元素节点上的 events 和 nativeEvents 生成 data 数据。genHandlers 方法遍历事件对象 events，对同一个事件名称的事件调用 genHandler(name, events[name]) 方法，它的内容看起来多，但实际上逻辑很简单，首先先判断如果 handler 是一个数组，就遍历它然后递归调用 genHandler 方法并拼接结果，然后判断 hanlder.value 是一个函数的调用路径还是一个函数表达式， 接着对 modifiers 做判断，对于没有 modifiers 的情况，就根据 handler.value 不同情况处理，要么直接返回，要么返回一个函数包裹的表达式；对于有 modifiers 的情况，则对各种不同的 modifer 情况做不同处理，添加相应的代码串。

Vue 支持 2 种事件类型，原生 DOM 事件和自定义事件，它们主要的区别在于添加和删除事件的方式不一样，并且自定义事件的派发是往当前实例上派发，但是可以利用在父组件环境定义回调函数来实现父子组件的通讯。另外要注意一点，**只有组件节点才可以添加自定义事件，并且添加原生 DOM 事件需要使用 native 修饰符；而普通元素使用 .native 修饰符是没有作用的，也只能添加原生 DOM 事件。**原生DOM事件和自定义事件在编译过程中就会被区分出来，之后进入组件渲染阶段，原生DOM事件是发生在组件渲染内部，但是自定义事件是发生在子组件内部，并且**原生DOM事件的add和remove是通过addEventListener和removeEventListener实现，但是自定义事件是通过事件中心的通信实现绑定和执行一次等功能**

原生 DOM 事件，在组件的 events module 中会注册 create 和 update 钩子。执行 updateDOMListeners，这个方法会统一调用updateListeners来取到实例上传入的on事件和oldOn事件，对on中没有注册过的事件执行注册，(注册的事件会被vue的globalErrorhandler统一包裹处理错误)。对于注册过的就替换fns函数(也就是被包裹过的函数)，oldOn有 on没有的事件进行remove。addEventListener 函数中的 handler 如果在 DOM 事件的回调函数执行期间如果修改了数据，那么这些数据更改推入的队列会被当做 macroTask 在 nextTick 后执行。

自定义事件 在子组件创建的时候挂载，拿到父组件的on事件之后传给子组件的listeners 然后父组件的所有nativeOn事件变成on事件，在组件的create和update生命周期更新。子组件拿到listeners之后，在initEvents中调用updateComponentListeners，在updateComponentListeners，创建target保存全局此时的event消息订阅中心，进行事件的绑定和分发
...
```
vue会在不同生命周期触发不同modules下相应生命周期的方法。
...
```
@click.native.prevent -> ['.native', '.prevent']
...
会根据 important 参数来将事件前置 比如 checkbox 的 change 事件比普通的change事件重要就前置了
```
export function addHandler (
  ...
  // 标准化 click.right 和 click.middle 因为它们实际上并没有触发
...
## initEvents
updateComponentListeners会有一个全局的target记录本次更新listener的vm是谁。在更新之后将target设置成null
```
...
updateComponentListeners中使用发布订阅监听事件和取消事件，拿到父组件绑定的on事件，来注册监听。否则子组件触发emit也没有用。
在updateListeners中 如果在旧的listeners中存在并且在新的listeners中也存在，那就更新，否在在新的listeners中不存在旧取消监听off。

因为是自定义实现的发布订阅，所以vue可以实现一些后缀符号的操作比如
.stop
.prevent
.self
.capture
