...
2. 合并options
3. initLifecycle阶段如果父组件不是内置组件就把本节点添加到父组件的$children中，否则添加到内置组件的父组件中，并且初始化组件生命周期的标志和watcher、children、refs、directInactive、inactive
```
function initMixin (Vue) {
  Vue.prototype._init = function (options) {
    var vm = this;
...
```
### updateDOMListeners
属于updateDOMListeners的事件有，input、checkbox等，因为一定要从浏览器自动监听触发事件。
```
  ...
  function add$1 (
    ...
    // 在修补期间附加到外部元素，并再次触发。这个
    // 发生是因为浏览器在事件传播之间触发微任务滴答。
...
function createPatchFunction (backend) {
    ...
    // 收集各个生命周期要做的事情，比如updateDOMListener就在create更新
...
```
## initRender
创建$createElement可以确定此时render的vm上下文。可以根据vm创建render function 创建属于这个vm的render watcher，绑定组件attrs和listeners的监听，如果变化可以引起组件的渲染更新
```
...
```
# eventsMixin
通过注册一个发布订阅模式实现事件的注册、执行、解绑。并且在emit事件的时候，会触发invokeWithErrorHandling，进行全局的错误收集。
```
function eventsMixin (Vue) {
  var hookRE = /^hook:/;
  Vue.prototype.$on = function (event, fn) {
    // 这里的this是Vue实例，因为$on虽然定义在Vue.prototype上，但是实际调用$on方法的是vue实例，所以可以拿到this
    var vm = this;
    if (Array.isArray(event)) {
      ...
      // 通过使用在注册时标记的布尔标志来优化 hook:event 成本。而不是哈希查找
