 ...
 */
 var lengthOfLIS = function(nums) {
    ...
    let res = []
    function getLength(idx, order) {
        if(idx === nums.length - 1) {
            if(nums[idx] > order[order.length - 1]) order.push(nums[idx])
            res.push(order)
            return
        }
        if(nums[idx] <= order[order.length - 1]) {
            getLength(idx+1, [nums[idx]]) 
            if(nums[idx] > order[order.length - 2]) order[order.length - 1] = nums[idx]
            getLength(idx+1, order)
            return
        }
        if(nums[idx] > order[order.length - 1]) {
            order.push(nums[idx])
